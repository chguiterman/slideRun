...
#       "pith/bark" into a legend.
  ...
  p <- (p + ggplot2::geom_point(data = events, ggplot2::aes_string(shape = 'rec_type',
                                  size = 'rec_type', fill = 'conf'))
                       #size = event_size, color = "black")
                       #color = "black")
          + ggplot2::scale_size_manual(values = event_size)
       #   + ggplot2::scale_colour_gradient(low="blue", high="red")
          + ggplot2::scale_fill_gradient(low="blue", high="white", guide="colorbar")
          + ggplot2::scale_shape_manual(guide = "legend",
                               values = c("Scar" = 25, "Injury" = 6, "Pith/Bark" = 20))) # `shape` 25 is empty triangles

  if (composite_rug) {
    comp <- burnr::composite(x, filter_prop = filter_prop,
                        filter_min_rec = filter_min_rec,
                        filter_min_events = filter_min_events,
                        injury_event = injury_event)
    p <- (p + ggplot2::geom_rug(data = rings[rings$year %in% burnr::get_event_years(comp, injury_event = injury_event)[['COMP']], ],
                       sides = "b", color = "black")
